# 🧑‍🔬 Research Review Dossier: Arav Jain
*   **Role**: Research Member
*   **Repository Component**: Literature Review (9 Papers)

This directory houses the systematic literature reviews for Arav Jain. Follow the links below to view or edit the respective paper reviews.

---

## 🔬 Section 1: Assigned Papers (Shared Reviews)

1.  **[Paper 1: Deep learning: new computational modelling techniques for genomics](paper-1.md)**
    *   *Nature Reviews Genetics (2019)* | DOI: `10.1038/nrg.2019.0122-6`
2.  **[Paper 2: A universal SNP and small-indel variant caller using deep neural networks](paper-2.md)**
    *   *Nature Biotechnology (2018)* | DOI: `10.1038/nbt.4235`
3.  **[Paper 3: An integrated encyclopedia of DNA elements in the human genome](paper-3.md)**
    *   *Nature (2012)* | DOI: `10.1038/nature11247`
4.  **[Paper 4: Cell2location maps fine-grained cell types in spatial transcriptomics](paper-4.md)**
    *   *Nature Biotechnology (2022)* | DOI: `10.1038/s41587-021-01139-4`

---

## 🔬 Section 2: Self-Selected Papers

5.  **[Paper 5: Implementation of a Disease Trigger Prediction Model Using AIML for Early Diagnosis of Epilepsy](paper-5.md)**
    *   *IEEE ICSTCE (2023)* | DOI: `10.1109/ICSTCE57884.2023.10111452`
6.  **[Paper 6: Artificial Intelligence and Machine Learning (AIML) Approaches to Empower Genomics, Drug, and Biomarker Discovery in ADRD](paper-6.md)**
    *   *Alzheimer's & Dementia (2021)* | DOI: `10.1002/alz.055848`
7.  **[Paper 7: A Machine Learning-Based System for Predicting Monogenic Genetic Disorders](paper-7.md)**
    *   *IEEE ICOEI (2023)* | DOI: `10.1109/ICOEI56765.2023.10125672`
8.  **[Paper 8: An Efficient Deep Convolutional Neural Networks Model for Genomic Sequence Classification](paper-8.md)**
    *   *IEEE ICBDS (2021)* | DOI: `10.1109/ICBDS53701.2021.9935914`
9.  **[Paper 9: Detecting Gene Sequence of Recurrence Cervical Cancer Interpolating Biomedical Sensors](paper-9.md)**
    *   *IEEE CSITSS (2022)* | DOI: `10.1109/CSITSS57431.2022.10031644`

---

## 🏁 Section 3: Synthesis Summary

*Leave this section blank until all reviews are complete. Compile these final conclusions to summarize your findings in this folder.*

### 🔍 Common Themes Across Reviewed Papers
*   *Theme 1: [your notes here]*
*   *Theme 2: [your notes here]*

### 🛠️ Key Methods/Tools That Appear Repeatedly
*   *Method/Tool 1: [your notes here]*
*   *Method/Tool 2: [your notes here]*

### 🕳️ Gaps in Literature Relevant to Our Project
*   *Gap 1: [your notes here]*
*   *Gap 2: [your notes here]*
